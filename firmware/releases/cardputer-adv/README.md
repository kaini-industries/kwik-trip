...
M5Burner's installed `burn()` implementation was also exercised with subprocess execution mocked for macOS, Linux and Windows. Each selected the factory file at `0x0`. This checks command construction without opening a serial port; it is not a GUI upload or physical flashing test.
